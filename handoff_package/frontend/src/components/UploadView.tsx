...
import { IconFile, IconPlay, IconUpload } from "./icons";
...
export function UploadView({ onSubmit, busy }: Props) {
  ...
  return (
    <form className="container-narrow" onSubmit={handleSubmit} style={{ paddingTop: 32 }}>
      <h1 className="t-h1" style={{ marginBottom: 8 }}>Extract a bank statement</h1>
      <p className="t-lead" style={{ marginBottom: 32 }}>
        Drop a PDF and we'll fan out a team of agents — layout classifier,
        account, summary, transactions — then verify, reconcile, and return
        structured JSON.
      </p>
      ...
      >
        <input
          ...
          onChange={pickPdf} disabled={busy} required
          style={{ position: "absolute", width: 1, height: 1, opacity: 0, pointerEvents: "none" }}
        />
        <div className="upload-icon-wrap">
          <IconUpload size={22} />
        </div>
        <div style={{ fontWeight: 600, fontSize: "var(--text-lg)", color: "var(--ink-1)", marginBottom: 6 }}>
        ...
        <div className="mono" style={{ fontSize: 11, color: "var(--ink-4)", marginTop: 10 }}>
          ...
          <label htmlFor="ocr-input" style={{ marginLeft: 6, color: "var(--accent)", borderBottom: "1px solid var(--accent-tint-2)", cursor: "pointer" }}>
            attach
          </label>
          <input
            id="ocr-input" type="file" accept=".txt,text/plain"
            onChange={pickOcr} disabled={busy}
            style={{ position: "absolute", width: 1, height: 1, opacity: 0, pointerEvents: "none" }}
          />
        </div>
      </label>
      ...
      <ol style={{ paddingLeft: 20, margin: 0, color: "var(--ink-2)", fontSize: "var(--text-sm)", lineHeight: 1.7 }}>
