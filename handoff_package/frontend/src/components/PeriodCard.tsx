...
export function PeriodCard({ period, initialOpen, transactions, onReview }: Props) {
  ...
  return (
    <section className={"period-card" + (open ? " open" : "")}>
      <div className="period-head" onClick={() => setOpen(!open)}>
        <div style={{ flex: 1, display: "flex", alignItems: "baseline", gap: 4, flexWrap: "wrap" }}>
          <span className="period-title">
            {month}<span className="last4">· ...{account.account_last4}</span>
          </span>
        ...
        {reconciliation.reconciled ? (
          ...
          <>
            ...
            <Button
              ...
              size="sm"
              onClick={(e) => { e.stopPropagation(); onReview?.(period); }}
        ...
        <span className="chev"><IconChevDown size={16} /></span>
      </div>
      ...
      <div className="period-stats">
        ...
        <div>
          ...
          <div className="stat-value tnum mono">
            {fmt$(summary.deposits_total)}<span className="small">· {summary.deposits_count}</span>
          </div>
        </div>
        <div>
          <div className="stat-label">Withdrawals</div>
          <div className="stat-value tnum mono">
            {fmt$(summary.withdrawals_total)}<span className="small">· {summary.withdrawals_count}</span>
          </div>
        </div>
      </div>
